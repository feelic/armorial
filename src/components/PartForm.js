...
export default function PartForm (props) {
  ...
  if (part.partitionType) {
    return <div className={styles.partForm}>
      <PartitionSelector onSelect={actions.changePartition} armsParts={armsParts} part={part}/>
      <button onClick={() => actions.clearPartition(part.id)} > clear partition </button>
      {part.parts.map(partId => {
        return <PartForm key={partId} actions={actions} armsParts={armsParts} part={armsParts[partId]} />
      })}
    </div>
  }
  if (part.field) {
  ...
  return (
    ...
    <FieldForm actions={actions} armsParts={armsParts} part={part} />
    Or create partition
    <PartitionSelector onSelect={actions.createPartition} armsParts={armsParts} part={part} />
    </div>
  );
}
...
function PartitionSelector (props) {
  ...
  <select onChange={(e) => onSelect(part.id, e.target.value)} value={part.partitionType}>
    <option value={null}> </option>
    {Object.values(partitions).map(partition => {
      return <option key={partition.id} value={partition.id}>
        {partition.label}
      </option>
    })}
  </select>
...
function FieldForm (props) {
  ...
  <select onChange={(e) => actions.changeField(part.id, e.target.value)} value={part.field}>
    <option value={null}> </option>
